...
[Hooks扩展资料](https://zh-hans.reactjs.org/docs/hooks-intro.html)

[定义hooks实现](https://github.com/streamich/react-use)
...
Redux是JavaScript应用的状态容器，实现原理是基于发布订阅
...
每次都重新调`render`和`getState`太low了，想用`react`的方式来写，需要`react-redux`的支持
